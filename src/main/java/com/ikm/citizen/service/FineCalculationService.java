...
public class FineCalculationService {
  public FineGenerationResponse generateFine(FineCalculationWrapper request){
    ...
    if (daysBetween < 0) {
      ...
      if (daysBetween > -29) {
        lateFee = Double.parseDouble(request.getLicenseFee().toString()) * 0.10; //Apply Before Valid To Date and After 29 days 10%
      }
      if(daysBetween<-29){
        double lateFeeSecond = Double.parseDouble(request.getLicenseFee().toString()) * 0.25; // Calculate First 10 Days 25%
        long additionalLateDays = (-1 * (daysBetween + 40)) / 15; // Calculate additional late days
